use http::{
    ...
    method, HttpTryFrom, Method, Request,
};
use std::{
    ...
    HeaderValue: HttpTryFrom<A>,
{
        ...
        I: IntoIterator<Item = A>,
    {
        let origins = iter
            .into_iter()
            .map(|v| HttpTryFrom::try_from(v).unwrap_or_else(|_| panic!("Invalid header value")))
            .collect();
...
/// Produces the most restrictive configuration
...
impl Config {
    ...
    pub fn process_request<B>(&self, request: &Request<B>) -> Result<CorsResource, InvalidRequest> {
        ...
        match (origin, request.method(), requested_method) {
            ...
            (Some(origin), &Method::OPTIONS, Some(requested_method)) => {
                ...
                // Most of the value computation could be pushed to `build` time

                let allowed_methods =
                    collect_header_value(&self.allowed_methods).expect("Invalid methods");
                headers.insert(header::ACCESS_CONTROL_ALLOW_METHODS, allowed_methods);

                let allowed_headers =
                    collect_header_value(&self.allowed_headers).expect("invalid headers");
                headers.insert(header::ACCESS_CONTROL_ALLOW_HEADERS, allowed_headers);

                if let Some(max_age) = self.max_age {
                    let max_age =
                        HeaderValue::try_from(max_age.as_secs()).expect("invalid max age");
            ...
            (Some(origin), _, _) => {
                ...
                if !self.exposed_headers.is_empty() {
                    let exposed_headers =
                        collect_header_value(&self.exposed_headers).expect("invalid headers");
                    headers.insert(header::ACCESS_CONTROL_EXPOSE_HEADERS, exposed_headers);
                }
    ...
    fn common_headers(&self, origin: HeaderValue) -> HeaderMap {
        ...
        if self.allow_credentials {
            ...
            let allowed_credentials = HeaderValue::from_static("true");
            headers.insert(
...
fn collect_header_value<I>(values: I) -> Result<HeaderValue, header::InvalidHeaderValue>
where
    I: IntoIterator,
    I::Item: AsRef<str>,
{
...
mod test {
    ...
    fn simple_response_includes_exposed_headers() -> TestResult {
        let cfg = CorsBuilder::new()
            .allow_origins(AllowedOrigins::Any { allow_null: false })
            .allow_methods(vec![Method::POST])
            .expose_headers(vec![header::WARNING, HeaderName::from_static("x-custom")])
            .into_config();
    ...
    fn common_compares_origin_against_allowed_origins(
        ...
        let cfg = cfg_builder
            .allow_origins(AllowedOrigins::from_iter(vec!["http://foo.example", "http://bar.example"]))
            .into_config();
